...
@section('content')
<div class="container">
    <form action="{{ url('consumption/update/'.$consumption->id) }}" method="post" id="addForm">
        @csrf
        <div class="container row single-page">
            <div class="col-md-6">
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="number" class="form-control" name="wb_slip_no" value="{{ $consumption->wb_slip_no }}"
                            required>
                    </div>
                </div>
                <div class="form-group row">
                        ...
                        <select name="location_id" id="location_id" class="form-control" required>
                            @foreach($locations as $item)
                                <option value="{{ $item->id }}"
                                    {{ $consumption->location_id == $item->id ? 'selected':'' }}>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-sm-8">
                        <select name="lot_number" id="lot_number" class="form-control" required>
                            @foreach($lot_numbers as $item)
                                <option value="{{ $item->id }}"
                                    {{ $consumption->lot_number == $item->id ? 'selected':'' }}>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="number" name="truck_no" id="truck_no" class="form-control"
                            value="{{ $consumption->truck_no }}" required>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="number" name="weight" id="weight" class="form-control"
                            value="{{ $consumption->weight }}" required>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="date" class="form-control" name="date" value="{{ $consumption->date }}" required>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <select name="wh_name" id="wh_name" class="form-control" required>
                            @foreach($wh_names as $item)
                                <option value="{{ $item->id }}"
                                    {{ $consumption->wh_name == $item->id ? 'selected':'' }}>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-sm-8">
                        <select name="variety" id="variety" class="form-control" required>
                            @foreach($varieties as $item)
                                <option value="{{ $item->id }}"
                                    {{ $consumption->variety == $item->id ? 'selected':'' }}>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="number" name="bags" id="bags" class="form-control" value="{{ $consumption->bags }}"
                            required>
                    </div>
                </div>
